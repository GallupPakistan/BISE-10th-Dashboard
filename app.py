...
st.markdown(
    ...
    [data-testid="stSidebar"] hr {{
        border-color: rgba(255,255,255,0.2); margin: 16px 0;
    }}
...
def pass_fail_hbar(df, label_col, title="", height=None):
    ...
    # #region agent log
    debug_log(
        "app.py:pass_fail_hbar",
        "Pass/fail bar layout",
        {
            "title": bool(title),
            "showlegend": False,
            "margin_t": fig.layout.margin.t,
            "margin_b": fig.layout.margin.b,
        },
        "K",
    )
...
def subject_pass_hbar(subjects, top_n=15):
    ...
    fig = go.Figure(
        go.Bar(
            ...
            text=[f"{v:.1f}%" for v in data["Pass %"]],
            textposition="outside",
        )
    )
    fig.update_layout(
        title=chart_title(f"Subject-wise Pass % (top {len(data)})"),
        xaxis=dict(range=[0, 105], title="Pass %"),
        yaxis=dict(automargin=True),
        height=max(420, 28 * len(data)),
        showlegend=False,
...
def district_pass_hbar(districts, top_n=12):
    ...
    fig.update_layout(
        title=chart_title(f"District / City Pass % (top {len(data)})"),
        xaxis=dict(range=[0, 105], title="Pass %"),
        yaxis=dict(automargin=True),
        height=max(380, 32 * len(data)),
        showlegend=False,
        margin=chart_margins(title="x", extra_right=40),
    )
    return style_fig(fig)
...
def trend_line_chart(df, title, y_col="Pass %", color=NAVY):
    ...
    fig.update_layout(
        ...
        height=400,
        showlegend=False,
        margin=dict(t=80, b=52, l=20, r=30),
    )
    return style_fig(fig)
...
def year_compare_chart(trend_df):
    ...
    fig.update_layout(
        title=chart_title("Passed vs Failed by Year"),
        barmode="stack",
        height=360,
        showlegend=False,
        margin=chart_margins(title="Passed vs Failed by Year"),
    )
    return style_fig(fig)


def subject_multi_year_trend_chart(df, title="Subject Pass % Trend"):
    """df needs columns: Year, Subject, Pass %. One spline line per subject."""
...
def subject_group_chart(df):
    ...
    fig.update_layout(
        title=chart_title("Pass % by Subject Group"),
        barmode="group",
        yaxis=dict(range=[0, 110]),
        height=max(420, 80 * len(pivot)),
...
def bubble_scatter_chart(x, y, size, text, title="", x_title="", y_title="Pass %", height=440):
    ...
    # Only label bubbles that are big enough or few enough in number to stay readable —
    # dense point sets fall back to hover-only labels to avoid overlapping text.
    show_labels = len(x) <= 8
    fig = go.Figure(
        go.Scatter(
            ...
            marker=dict(
                size=size, sizemode="area", sizeref=2.0 * max_size / (46.0 ** 2), sizemin=6,
...
def stacked_area_chart(pivot_df, title="", y_title="Appeared", height=420):
    ...
    fig.update_layout(
        title=chart_title(title),
        xaxis=dict(dtick=1, title="Year"), yaxis=dict(title=y_title),
        height=height, legend=legend_bottom_clear(extra_rows=max(0, len(pivot_df.columns) // 5)),
        margin=chart_margins(title=title, legend_pos="bottom_multi"),
    )
    return style_fig(fig)
...
def grouped_bar_chart(x, series, title="", y_title="Students", height=400, colors=None):
    ...
    fig.update_layout(
        title=chart_title(title), barmode="group",
...
def heatmap_chart(z, x_labels, y_labels, title="", height=420):
    ...
    fig.update_layout(
        title=chart_title(title), xaxis=dict(dtick=1, title="Year"), yaxis=dict(automargin=True),
...
def build_subject_year_trend(board_sheets):
    """Stack extract_subject_data(board_sheets, y) across every available year."""
    frames = []
...
def render_overview(boards, year):
    ...
    kpi = [
        ("Boards", f"{df['Board'].nunique()}", "All BISE boards"),
    ...
    cols = st.columns(4)
    accents = [NAVY, ACCENT, PASS_COLOR, FAIL_COLOR]
    for col, accent, (label, val, sub) in zip(cols, accents, kpi):
    ...
    with c2:
        ...
        share_fig = donut_pie(
            rankings["Board"].tolist(),
            rankings["Appeared"].tolist(),
            PALETTE[: len(rankings)],
            "Students by Board",
            height=440,
        )
        show_chart(share_fig)
        debug_log(
            "app.py:overview_share",
            "Overview share chart",
            {
                "boards": len(rankings),
                "chart_type": "pie",
                "legend_orientation": share_fig.layout.legend.orientation if share_fig else None,
                "margin_b": share_fig.layout.margin.b if share_fig else None,
            },
            "J",
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    for i, board in enumerate(trend["Board"].unique()):
        ...
        fig.add_trace(
            go.Scatter(
                x=b["Year"], y=b["Pass %"], mode="lines", name=board,
                line=dict(width=3, shape="spline", smoothing=0.8, color=PALETTE[i % len(PALETTE)]),
    ...
    # --- Chart 4: Stacked Area — Appeared by board across years ---
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    ...
    # --- Chart 5: Grouped Bar — Passed vs Failed per board ---
    ...
    # --- Chart 8: Funnel — overall Appeared -> Passed ---
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    ...
    # --- Chart 9: Heatmap — Board x Year pass % ---
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    ...
    show_chart(heatmap_chart(heat_pivot.values, [str(c) for c in heat_pivot.columns], heat_pivot.index.tolist(), "Pass % Heatmap — Board × Year"))
    st.markdown("</div>", unsafe_allow_html=True)
...
def render_board_page(
    ...
    kpi_accents = [NAVY, PASS_COLOR, FAIL_COLOR, MALE_COLOR]
    kpi_data = [
    ...
    # --- Chart: Gauge — overall pass % for this board ---
    ...
    if not subject_groups.empty and "Appeared" in subject_groups.columns:
        ...
        show_chart(treemap_chart(districts["District"].tolist(), districts.get("Appeared", districts.get("Pass %")).tolist(), "Share by District"))
        st.markdown("</div>", unsafe_allow_html=True)

    if not subjects.empty:
        # --- Chart: Bubble Scatter — subject Appeared vs Pass % ---
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        ...
        show_chart(bubble_scatter_chart(
            subj_size, subjects["Pass %"].tolist(), subj_size, subjects["Subject"].tolist(),
            "Subject Appeared vs Pass % (bubble size = Appeared)", x_title="Appeared",
        ))
        st.markdown("</div>", unsafe_allow_html=True)

        # --- Chart: Multi-line — Subject Pass % across years (filterable via sidebar) ---
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.subheader("📈 Subject Pass % Trend (Multi-Year)")

        with st.sidebar:
            st.markdown('<div class="sidebar-section">Subject Trend Filters</div>', unsafe_allow_html=True)
            board_options = sorted(board_map.keys()) if board_map else [selected_board_name]
            default_boards = [selected_board_name] if selected_board_name in board_options else board_options[:1]
            picked_boards = st.multiselect(
                "🏫 Board(s)", options=board_options, default=default_boards,
                key=f"subj_trend_boards_{selected_board_name}",
            )
            if not picked_boards:
                picked_boards = default_boards

            multi_board_frames = []
            for b_name in picked_boards:
                b_sheets = board_sheets if b_name == selected_board_name else boards[board_map[b_name]]
                b_df = build_subject_year_trend(b_sheets)
                if not b_df.empty:
                    b_df = b_df.copy()
                    b_df["Board"] = b_name
                    multi_board_frames.append(b_df)
            subj_trend_df = (
                pd.concat(multi_board_frames, ignore_index=True) if multi_board_frames
                else pd.DataFrame(columns=["Year", "Subject", "Pass %", "Appeared", "Board"])
            )
            ...
            if subj_trend_df.empty:
                ...
                avail_trend_years = sorted(subj_trend_df["Year"].unique())
                if len(avail_trend_years) > 1:
                    yr_range = st.select_slider(
                        "📅 Year range", options=avail_trend_years,
                        value=(avail_trend_years[0], avail_trend_years[-1]),
                        key=f"subj_trend_years_{selected_board_name}",
                    )
                else:
                    yr_range = (avail_trend_years[0], avail_trend_years[0])
                pct_range = st.slider(
                    "🎯 Pass % range",
                    min_value=0, max_value=100, value=(0, 100),
                    key=f"subj_trend_pct_{selected_board_name}",
                    help="Keeps subjects whose average pass % over the selected years falls in this range.",
                )

        if subj_trend_df.empty:
            st.info("No multi-year subject data available for the selected board(s).")
        else:
            subj_trend_df = subj_trend_df[(subj_trend_df["Year"] >= yr_range[0]) & (subj_trend_df["Year"] <= yr_range[1])]

            avg_pct = subj_trend_df.groupby("Subject")["Pass %"].mean()
            keep_subjects = avg_pct[(avg_pct >= pct_range[0]) & (avg_pct <= pct_range[1])].index.tolist()
            subj_trend_df = subj_trend_df[subj_trend_df["Subject"].isin(keep_subjects)]
    ...
    debug_log("app.py:charts_row", "Two-chart rows rendered", {"gender_pie": not gender_df.empty, "type_bar": not type_df.empty}, "I")
    ...
    if not subjects.empty:
        ...
        if not weak.empty:
            ...
            st.dataframe(weak[["Subject", "Pass %"] + [c for c in ["Appeared", "Passed"] if c in weak.columns]], use_container_width=True, hide_index=True)
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    if not districts.empty:
        ...
        show_chart(district_pass_hbar(districts))
    ...
    if not subjects.empty:
        ...
        show_chart(subject_pass_hbar(subjects))
        csv_download_button(subjects, "⬇️ Download subjects CSV", f"{selected_board_name}_subjects.csv")
        debug_log("app.py:subject_bar", "Subject horizontal bar rendered", {"subjects": len(subjects), "chart_type": "hbar"}, "I")
        st.markdown("</div>", unsafe_allow_html=True)

    debug_log("app.py:board", "Board page rendered", {"board": selected_board_name, "insights": len(insights), "pie_sections": 2, "subject_chart": "hbar_full"}, "H")
...
def render_compare_page(boards, board_map, selected_names, year):
    ...
    for i, b in enumerate(selected_names):
        ...
        trend_fig.add_trace(
            go.Scatter(
                x=bd["Year"], y=bd["Pass %"], mode="lines+markers", name=b,
                line=dict(width=3, shape="spline", smoothing=0.8, color=PALETTE[i % len(PALETTE)]),
                marker=dict(size=8),
            )
        )
    trend_fig.update_layout(
        ...
        yaxis=dict(title="Pass %", showgrid=True, gridcolor="rgba(0,0,0,0.06)", griddash="dot"),
        legend=legend_top_right(),
        margin=chart_margins(legend_pos="top"),
        hovermode="x unified",
    )
    ...
    with gc1:
        ...
        show_chart(grouped_bar_chart(
            agg["Board"].tolist(), {"Passed": agg["Passed"].tolist(), "Failed": agg["Failed"].tolist()},
    ...
    with gc2:
        ...
        show_chart(bubble_scatter_chart(
            agg["Appeared"].tolist(), agg["Pass %"].tolist(), agg["Appeared"].tolist(), agg["Board"].tolist(),
            "Appeared vs Pass % (bubble size = Appeared)", x_title="Total Appeared",
        ))
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if gender_rows:
        ...
        pivot = gdf.pivot_table(index="Board", columns="Gender", values="Pass %", aggfunc="mean")
        pivot = pivot.reindex(selected_names)
        gender_colors = [GENDER_COLORS.get(c, NAVY) for c in pivot.columns]
        st.markdown('<div class="section-card">', unsafe_allow_html=True)
        st.subheader("👥 Gender Pass % Comparison")
        show_chart(grouped_bar_chart(
            pivot.index.tolist(), {col: pivot[col].round(1).tolist() for col in pivot.columns},
...
board_list = ["Overview — All Boards"] + sorted(board_map.keys())

debug_log("app.py:load", "Workbook loaded", {"board_count": len(board_prefixes), "punjab_file": str(data_loader._punjab_path())}, "A")
...
with st.sidebar:
    ...
    st.markdown(
        ...
        <div class="sidebar-active-value">{selected_board_name}<br>{year_label}</div>
        </div>""",
        unsafe_allow_html=True,
    )
    debug_log("app.py:sidebar", "Sidebar filters rendered", {"board": selected_board_name, "year": year_label, "boards": len(board_list)}, "S")
...
if selected_board_name == "Overview — All Boards":
    ...
    debug_log(
        "app.py:board_totals",
        "Board totals for selection",
        {
            "board": selected_board_name,
            "year": year,
            "available_years": get_available_years(board_sheets),
            "appeared": totals["appeared"],
            "passed": totals["passed"],
            "pass_pct": totals["pass_pct"],
            "boys": int(gender_df.loc[gender_df["Gender"] == "Male", "Appeared"].sum()) if not gender_df.empty else 0,
            "girls": int(gender_df.loc[gender_df["Gender"] == "Female", "Appeared"].sum()) if not gender_df.empty else 0,
            "gender_rows": len(gender_df),
            "type_rows": len(type_df),
            "type_cols": list(type_df.columns),
        },
        "B",
        run_id="post-fix",
    )
